...
## CI-fix — @MainActor ontbrak op WorkoutPlaybackControllerTests

CI-run op de Optie B-patch faalde met een reeks
`main actor-isolated ... in een synchronous nonisolated context`-fouten.

**Oorzaak:** `WorkoutPlaybackController` is `@MainActor` (nodig voor
`ObservableObject`/SwiftUI), maar de testfuncties die 'm aanriepen waren
zelf niet `@MainActor` en niet `async` — dezelfde categorie fout als de
eerdere Sprint 3 CI-fix (`XCTAssertEqual(await ...)`), nu in een andere
vorm.
